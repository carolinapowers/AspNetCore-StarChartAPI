...
namespace StarChartTests
{
    public class ConfigureMiddlewareTests
    {
        ...
        public void CallAddApplicationDbContextInConfigureServicesTest()
        {
            ...
            Assert.True(rgx.IsMatch(file), "`Startup.ConfigureServices` didn't contain a call for `AddDbContext` on `services` with the type arguement `ApplicationDbContext`");
        }
